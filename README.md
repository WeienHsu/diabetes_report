...
2. **匯出飲食**：Google Sheet「飲食與三大營養素紀錄表」
   - 電腦：**切到「飲食日誌」分頁** → 檔案 → 下載 → 逗號分隔值
   - 手機：Sheets app → 分享與匯出 → 另存為 **xlsx**（整本活頁簿，不必挑分頁）
3. 開 https://cgm.whtwbrown.com ，兩個檔案丟上去，選期間，按產生
4. 報告直接在畫面上讀；要紙本就按「下載 PDF」

手機也走得完——iOS Safari 的檔案選擇器讀得到「檔案」app，兩份匯出檔都在那裡。

> CSV 一次只會匯出**當前所在的那一頁**。停在「食物資料庫」匯出的話會解析出
> 0 餐，網頁會直接告訴你。xlsx 沒有這個問題：兩頁都在檔案裡，程式自己挑
> 「飲食日誌」。
